...
> & {
  ...
  globalDrop?: boolean;
  // Optional: register drag/drop listeners on a specific element (e.g., chat card)
...
export const PromptInput = ({
  ...
  const matchesAccept = useCallback(
    (f: File) => {
      ...
      const matchToken = (tok: string) => {
        ...
        if (tok.includes("/")) return type === tok; // e.g. application/pdf
        // bare extension like 'pdf' or 'txt'
        return name.endsWith(`.${tok}`);
      };

      return tokens.some((t) => matchToken(t));
    },
    [accept]
  );
  ...
  // Optional: attach drag/drop handlers to a specific target element (panel)
  useEffect(() => {
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = (event) => {
    ...
    // Be robust: read text via FormData, fallback to named control if present
    const fd = new FormData(event.currentTarget);
    const text = (fd.get('message') ?? (event.currentTarget as any).message?.value ?? '').toString();
